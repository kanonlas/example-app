            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('diary.update', $diaryEntry) }}">
                        ...
                        <!-- {{-- Emotion --}}  -->
                        <div class="mb-4"> 
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select Emotions</label>
                                    ...
                                    <div class="grid grid-cols-1 gap-4"> 
                                        @foreach ($emotions as $emotion) 
                                            <div class="flex items-center mb-4"> 
                                                ...
                                                <input type="checkbox" id="emotion_{{ $emotion->id }}" name="emotions[]" value="{{ $emotion->id }}" class="h-5 w-5 text-indigo-600 border-gray-300 rounded dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-indigo-600" onchange="toggleIntensityInput({{ $emotion->id}})">
                                                <label for="emotion_{{ $emotion->id }}" class="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300">{{ $emotion->name }}</label>
                                            
                                                <!-- Intensity input container, initially hidden --> 
                                                <div class="ml-4 {{ in_array($emotion->id, old('emotions', $diaryEntry->emotions->pluck('id')->toArray())) ? '' : 'hidden' }}" id="intensity_container_{{ $emotion->id }}"> 
                                                <input type="number" name="intensity[{{ $emotion->id }}]" class="w-full border-gray-300 rounded-md shadow-sm dark:bg-gray-700 dark:text-gray-100 dark:border-gray-600 focus:ring-indigo-500 focus:border indigo-500" placeholder="Intensity" min="1" max="10" value="{{ old('intensity.' . $emotion->id, $diaryEntry->emotions->find($emotion->id)->pivot->intensity ?? '') }}">
                                                </div>
